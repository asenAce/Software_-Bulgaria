...
<!-- END OF RATING-->  
  ...
  <?php
  					$querySelectAllComments = "SELECT * FROM comments WHERE comment_movie_title LIKE '%$movie_title%'";
  						$showTheComments=$theConnection->executeTheQuery($querySelectAllComments);
							while($row=mysqli_fetch_assoc($showTheComments)){
								$commentID=$row['comment_id'];
								$Theuser=$row['comment_author'];
								$TheComment = $row['comment_content'];
						
  ?>                        
                            <div class="panel-heading" style=" margin-top:10px;"><i class="glyphicon glyphicon-user" style="margin-right:3px;"></i><span><?php echo $Theuser; ?></span> </div>
